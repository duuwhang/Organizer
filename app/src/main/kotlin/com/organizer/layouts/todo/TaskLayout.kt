...
open class TaskLayout(val parentLayout: ToDoLayout, val identifier: Int, displayText: String) :
    ...
    private fun updateFolderCompletions() {
        ...
        for (i in 0 until preferences.getInt("folderCount", 0)) {
            val folder = preferences.getString("folder$i", "Add Tasks;;0")!!.split(";;").toTypedArray()
            if (folder.size > 2) {
                done = true
                for (j in 2 until folder.size) {
                    if ("0" == preferences.getString(folder[j], "")!!.split(";;").toTypedArray()[1]) {
                        done = false
                        break
                    }
                }
    ...
    private fun updateFolderCompletions2() {
        ...
        val folders = preferenceService.restoreFolders()
        val tasks = preferenceService.restoreTasks()

        folders.forEach {
//            val attributes = it.split(";;")
//            attributes.subList(2, attributes.size - 1).any {  }
        }
